...
impl FromStr for GlobalState {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        serde_json::from_str(s)
            .unwrap_or_else(|_| return Err(String::from("Failed to parse GlobalState from JSON")))
    }
}
...
static GLOBAL_STATE: LazyLock<Arc<Mutex<GlobalState>>> = LazyLock::new(|| {
    Arc::new(Mutex::new(GlobalState {
        ...
        last_updated: SystemTime::now()
            ...
            .expect("Could not determine time since UNIX_EPOCH")
            .as_millis() as u64,
    }))
});
...
async fn fetch_global_state(uuid: String) -> Result<Option<GlobalState>, ServerFnError> {
    ...
    if client_search_result.is_none() {
        return Err(ServerFnError::MissingArg(String::from(
...
async fn register_user() -> Result<Client, ServerFnError> {
    let name = create_name()
        .await
        .unwrap_or_else(|_| String::from("IO Error :D"));
...
async fn update_color(uuid: String, color: u16) -> Result<(), ServerFnError> {
    ...
    if client_search_result.is_none() {
        return Err(ServerFnError::MissingArg(String::from(
            "owwwo you dwo nwot hwave an acwount :3",
        )));
    }
...
async fn create_name() -> Result<String, std::io::Error> {
    let mut nouns: Vec<String> = std::fs::read_to_string("nouns.csv")?
        .split("\r\n")
        .map(|str| String::from(str))
        .collect();
    let mut adjectives: Vec<String> = std::fs::read_to_string("adjectives.csv")?
        .split("\r\n")
        .map(|str| String::from(str))
        .collect();
...
async fn delete_inactive_clients() {
    ...
    state
        .clients
        .retain(|client| time_now - client.last_ping < TIMEOUT_MS);
}
...
fn hue_to_hex(hue: u16) -> String {
    ...
    let rgb: (u8, u8, u8) = (
        (rgb.0 * 255.0f64) as u8,
        (rgb.1 * 255.0f64) as u8,
        (rgb.2 * 255.0f64) as u8,
    );
...
fn HomePage() -> impl IntoView {
    ...
    let send_color_update = move || {
        spawn_local(async move {
            ...
            let _ = update_color(
                client.get_untracked().unwrap().uuid.clone(),
                color.get_untracked(),
            )
            .await;
        })
